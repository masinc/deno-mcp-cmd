...
const CommandPatternSchema = z.union([
  ...
  description:
    "Pattern for matching command names with various matching strategies",
...
const ArgsPatternSchema = z.object({
  ...
  containsNone: z.array(z.string()).optional().meta({
    description:
      "Match only if args contain none of these strings (safe filtering)",
  ...
  startsWith: z.array(z.string()).optional().meta({
    description:
      "Match if the first argument starts with any of these prefixes (OR logic)",
  ...
  regexAll: z.array(z.string()).optional().meta({
    description:
      "Match if all these regex patterns are found in args (AND logic)",
  ...
  description:
    "Pattern for matching command arguments with various filtering options",
  examples: [
    { containsAny: ["--force"] },
...
const PathPatternSchema = z.object({
  ...
  regex: z.string().optional().meta({
    description:
      "Match if current working directory matches this regex pattern",
  ...
  isSubdirectory: z.boolean().optional().meta({
    description:
      "Match if current working directory is a subdirectory (not the root or exact match)",
    examples: [true, false],
    note:
      "When true, matches only if cwd is deeper than the initial working directory",
  }),
}).meta({
  ...
  spec: z.object({
    ...
    args: ArgsPatternSchema.optional().meta({
      ...
      examples: [
        { containsAny: ["--force", "-f"] },
        {
          startsWith: ["status", "log", "show"],
          containsNone: ["-n", "--dry-run"],
        },
      ],
...
const ApproveCommandRuleSchema = BaseRuleSchema.extend({
  ...
  description:
    "Rule that automatically approves command execution when patterns match",
  category: "convenience",
  severity: "low",
  useCases: [
    "Streamlining safe operations",
    "Auto-approving read-only commands",
...
const WarningRuleSchema = BaseRuleSchema.extend({
  kind: z.literal("WarningRule").meta({
    description:
      "Rule type identifier for warning about potentially dangerous commands",
  ...
  spec: z.object({
    ...
    acknowledgedAction: z.enum(["skip", "confirm", "approve"]).default("skip")
      .meta({
    ...
    description:
      "Specification for warning conditions and acknowledgment behavior",
    workflow:
      "Pattern match → Show warning → User acknowledges → Take acknowledgedAction",
  }),
}).meta({
  description:
    "Rule that shows warnings for potentially dangerous operations with configurable acknowledged action",
  category: "safety",
  severity: "medium",
  useCases: [
    "Warning about risky operations",
    "Requiring explicit confirmation",
    "Documenting dangerous actions",
  ],
  examples: [
    {
...
const ConfirmCommandRuleSchema = BaseRuleSchema.extend({
  ...
  spec: z.object({
    ...
    reason: z.string().optional().meta({
      description:
        "Human-readable explanation for why confirmation is required",
      examples: [
...
const ConditionalRuleSchema = BaseRuleSchema.extend({
  ...
  examples: [
    ...
    },
    {
      ...
      spec: {
        condition:
          "args.includes('--force') && args.includes('--recursive') && cwd && cwd.startsWith('/usr')",
        action: "block",
...
export const UserRulesConfigSchema = z.object({
  ...
  documentation:
    "See MCP-CMD documentation for complete rule configuration guide",
  examples: [
    {
      rules: [
        {
